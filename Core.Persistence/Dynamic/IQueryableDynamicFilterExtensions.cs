    ...
    private static IQueryable<T> Sort<T>(IQueryable<T> queryable, IEnumerable<Sort> sort)
    {
        foreach (Sort item in sort)
        {
            ...
            if (string.IsNullOrEmpty(item.Dir) || !_orders.Contains(item.Dir))
                throw new ArgumentException("Geçersiz sıralama tipi. Asc veya Desc olabilir.");
        }
    ...
    public static string Transform(Filter filter, IList<Filter> filters)
    {
        ...
        if (filter.Logic is not null && filter.Filters is not null && filter.Filters.Any())
        {
            if (!_logics.Contains(filter.Logic))
                throw new ArgumentException("Geçersiz mantık. Or veya And olabilir onun dışında olamaz!!!!.");
